...
export const ingredientSchema = object({
  ...
  unitType: unitTypeSchema.required().label('unit of measurement'),
  cost: number().min(0).required().label('cost per unit of measurement'),
})
...
if (import.meta.vitest) {
  ...
  it('can be valid', () => {
    expect(ingredientSchema.validateSync(fields)).toEqual({
      name: 'Honey',
      unitType: 'spoons',
      cost: 0.3,
    })
  ...
  it('is invalid without a name', () => {
    expect(() =>
      ingredientSchema.validateSync({ ...fields, name: '' }),
    ).toThrow()
  })

  it('is invalid without a unit type', () => {
    expect(() =>
      ingredientSchema.validateSync({ ...fields, unitType: '' }),
    ).toThrow()
  })

  it('is invalid with a wrong unit type', () => {
    expect(() =>
      ingredientSchema.validateSync({ ...fields, unitType: 'unknown' }),
    ).toThrow()
  })

  it('is invalid without a cost', () => {
    expect(() =>
      ingredientSchema.validateSync({ ...fields, cost: '' }),
    ).toThrow()
  })
